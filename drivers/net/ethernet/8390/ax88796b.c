...
module_param_string(mem, mem, sizeof(mem), 0644);
module_param_string(irq, irq, sizeof(irq), 0644);
module_param_string(media, media, sizeof(media), 0644);

MODULE_PARM_DESC(mem, "Port offset(s) from base");
MODULE_PARM_DESC(irq, "IRQ number(s)");
MODULE_PARM_DESC(media, "Media Mode(0=auto, 1=100full, 2=100half, 3=10full, 4=10half)");


static void *device0_base8;
			   ...
			   struct sk_buff *skb, int ring_offset)
{
   ...
   while (( inb((unsigned long)nic_base+EN0_DSR) & 0x20) ==0);	
...
static int ax_init_dev(struct net_device *dev)
{
   ...
   ei_local->bigendian = (ax->plat->flags & AXFLG_BIGENDIAN)? 1:0;
...
static int ax_probe(struct platform_device *pdev)
{
   ...
   if (pdev->id == 0) {
      ...
      mem8 = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	   ...
	   mem_size = resource_size(mem8);
     	
	   if (!request_mem_region(mem8->start, mem_size, pdev->name)) {
		   dev_err(&pdev->dev, "cannot reserve registers\n");
		   ret = -ENXIO;
		   goto exit_mem;
	   }

	   ei_local->mem = ioremap_nocache(mem8->start, mem_size);
	   ...
	   device0_base8 = ei_local->mem;

   } else                
      ei_local->mem = device0_base8;
   ...
   dev->base_addr = (unsigned long)ei_local->mem + my_mem;
 ...
 exit_req:
   if (pdev->id == 0)
	   release_mem_region(mem8->start, mem_size);
